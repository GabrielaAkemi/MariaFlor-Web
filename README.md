# 🌿 Maria Flor Canábica — Website Institucional

Bem-vindo(a) ao repositório do **Maria Flor Canábica Web**, uma plataforma institucional voltada à **educação, conscientização e acesso seguro à cannabis medicinal**.

O website foi desenvolvido para fortalecer a presença digital da associação, conectar pacientes, médicos e interessados no uso terapêutico da planta, de forma moderna, responsiva e acessível. 🌱

---

## 💡 Objetivo do Projeto

O **Maria Flor Web** tem como propósito **informar, educar e orientar** a sociedade sobre o uso medicinal da cannabis, promovendo conhecimento responsável e transparente.

A plataforma oferece:

- 📖 **Conteúdos educativos** sobre cannabis medicinal  
- 🧠 **Informações sobre patologias e possíveis tratamentos**  
- 🏥 **Apresentação institucional da associação**  
- 📄 **Explicações sobre atuação e regulamentação**  
- 💬 **Canal de contato para suporte e informações**  

---

## 🌐 Características do Website

- 📱 Totalmente responsivo (Desktop, Tablet e Mobile)
- ⚡ Navegação intuitiva e rápida
- 🎯 Estrutura clara e organizada
- 🔒 Conteúdo institucional e informativo
- ♿ Foco em acessibilidade e legibilidade

---
## 🚀 Como Executar o Projeto

1. **Clone o repositório:**

   ```bash
   git clone https://github.com/GabrielaAkemi/MariaFlor-Web.git
Acesse a pasta do projeto:

cd MariaFlor-Web
Abra o arquivo principal:

Abra o index.html no navegador
ou utilize uma extensão como Live Server.

📊 Status do Projeto
🚧 Em desenvolvimento

Versão atual: v1.0.0

🎨 Identidade Visual
O design do Maria Flor Web foi desenvolvido para transmitir:

🌿 Naturalidade e responsabilidade

💚 Confiança e acolhimento

📚 Seriedade institucional

🌐 Modernidade digital

⚖️ Licença
Este projeto é de uso institucional exclusivo da Maria Flor Canábica.

Não é permitida a reprodução, redistribuição ou utilização comercial do código ou da identidade visual sem autorização formal da instituição.

📜 Política de Uso
O conteúdo disponibilizado no website possui caráter informativo e educacional, voltado à conscientização sobre o uso medicinal da cannabis.

As informações apresentadas não substituem orientação médica profissional.

O uso indevido da marca, identidade visual ou conteúdo institucional poderá resultar em responsabilização conforme a legislação vigente.

👩‍💻 Equipe
Desenvolvido por: Gabriela Akemi

Projeto Institucional: Maria Flor Canábica

Categoria: Website Institucional Educacional

Ano: 2025

“Cuidar com conhecimento é florescer com saúde.” 🌸
— Maria Flor Canábica

